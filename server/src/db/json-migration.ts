...
export function migrateJsonToSqlite(options: JsonMigrationOptions): JsonMigrationResult {
  ...
  const existing = options.database.prepare("SELECT statistics_json FROM migration_runs WHERE source_sha256 = ? AND status = 'completed' ORDER BY completed_at DESC LIMIT 1").get(sourceSha256) as { statistics_json: string } | undefined;
  if (existing) {
    return { status: "already_migrated", sourceSha256, schemaVersion: migrationResult.currentVersion, statistics: JSON.parse(existing.statistics_json) as MigrationStatistics, warnings: [] };
  }
  ...
  try {
    options.database.prepare("INSERT INTO migration_runs(id, source_name, source_sha256, target_schema_version, status, statistics_json, started_at, completed_at) VALUES (?, ?, ?, ?, 'started', ?, ?, NULL)").run(runId, options.sourceName, sourceSha256, migrationResult.currentVersion, JSON.stringify(statistics), now);
    ...
    options.database.prepare("UPDATE migration_runs SET status = 'completed', statistics_json = ?, completed_at = ? WHERE id = ?").run(JSON.stringify(statistics), new Date().toISOString(), runId);
...
function migrateUsersAndTokens(database: DatabaseSync, workbench: LegacyWorkbench, now: string, statistics: MigrationStatistics): Map<string, MemberIdentity> {
  ...
  const insertAccessToken = database.prepare("INSERT INTO access_tokens(id, user_id, token_hash, created_at, expires_at, revoked_at) VALUES (?, ?, ?, ?, NULL, NULL)");
...
function migrateApiToken(database: DatabaseSync, workbench: LegacyWorkbench, now: string): void {
  ...
  database.prepare("INSERT INTO api_tokens(id, name, token_hash, created_at, expires_at, revoked_at) VALUES (?, 'legacy-incoming-api', ?, ?, NULL, NULL)").run("api-legacy-incoming", sha256(rawToken), now);
}

function migrateTasks(database: DatabaseSync, workbench: LegacyWorkbench, identities: Map<string, MemberIdentity>, now: string, statistics: MigrationStatistics, warnings: string[]): void {
  const insertTask = database.prepare("INSERT INTO tasks(id, title, description, priority, status, progress, due_date, owner_id, created_by, source, is_private, created_at, updated_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
  const insertLog = database.prepare("INSERT INTO task_progress_logs(id, task_id, author_id, author_name, content, progress_snapshot, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
  const insertComment = database.prepare("INSERT INTO task_comments(id, task_id, author_id, author_name, author_role, content, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
  ...
  for (const task of workbench.tasks) {
    ...
    const assignedIdentity = assignee === "me" ? identities.get("owner") : assignee.startsWith("assistant:") ? identities.get(assignee.slice("assistant:".length)) : undefined;
    ...
    insertTask.run(id, title, task.desc ?? "", task.priority, normalizedStatus.status, progress, task.due ?? null, ownerId, creator, source, Number(assignee === "me"), createdAt, completedAt ?? createdAt, completedAt);
    statistics.tasks += 1;
    ...
    for (const [index, log] of (task.logs ?? []).entries()) {
      ...
      const snapshot = typeof log.progress === "number" && Number.isInteger(log.progress) && log.progress >= 0 && log.progress <= 100 ? log.progress : null;
    ...
    for (const [index, comment] of (task.comments ?? []).entries()) {
      ...
      insertComment.run(`${id}:comment:${index}`, id, author?.id ?? null, comment.by ?? null, role, content, isoFromMillis(comment.t, createdAt));
...
function migrateTodos(database: DatabaseSync, workbench: LegacyWorkbench, now: string, statistics: MigrationStatistics): void {
  const insert = database.prepare("INSERT INTO todos(id, content, todo_date, is_completed, completed_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
  for (const todo of workbench.todos) {
    ...
    insert.run(assertNonEmptyString(todo.id, "todo id"), assertNonEmptyString(todo.text, "todo content"), todo.date ?? null, Number(todo.done), completedAt, createdAt, completedAt ?? createdAt);
    statistics.todos += 1;
  }
}
...
function migrateNotes(database: DatabaseSync, workbench: LegacyWorkbench, now: string, statistics: MigrationStatistics): void {
  ...
  for (const note of workbench.notes) {
    ...
    insert.run(assertNonEmptyString(note.id, "note id"), assertNonEmptyString(note.text, "note content"), Number(note.pinned ?? false), createdAt, createdAt);
    statistics.notes += 1;
  }
}
...
function migrateFiles(database: DatabaseSync, workbench: LegacyWorkbench, now: string, statistics: MigrationStatistics): void {
  ...
  for (const file of workbench.files) {
    ...
    insert.run(assertNonEmptyString(file.id, "file id"), assertNonEmptyString(file.name, "file name"), assertNonEmptyString(file.path, "file path"), createdAt, createdAt);
    statistics.files += 1;
  }
}
