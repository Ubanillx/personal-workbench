...
import { createFile, createUser, deleteFile, getAccessInfo, getFiles, getMe, getReview, getUsers, importInbox, logout, previewInbox, rotateUserToken, updateUser, useFile, type AccessInfo, type Collaborator, type ImportantFile, type InboxDraft, type ReviewData } from "../services/apiClient";
...
export function FilesPage(): React.ReactElement {
  ...
  const copy = (file: ImportantFile): void => { if (!navigator.clipboard) { window.prompt("请复制以下路径", file.filePath); return; } void navigator.clipboard.writeText(file.filePath).then(() => useFile(file.id)).then((updated) => setFiles((items) => items.map((item) => item.id === updated.id ? updated : item))).catch(() => window.prompt("请复制以下路径", file.filePath)); };
