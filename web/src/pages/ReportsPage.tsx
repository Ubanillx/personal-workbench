  ...
  if (loading)
    return (
      <section className="page-section">
        <div className="loading-inline">正在加载周报…</div>
      </section>
    );
      ...
      {me && (
        <UploadForm
          ...
          onDone={(next) => {
            ...
            setNotice(me.role === "owner" ? "已上传并提交审核" : "已提交审核");
          }}
          onError={setError}
        />
      )}
...
function UploadForm({
  ...
  const submit = (event: React.FormEvent): void => {
    ...
    void uploadReport({ file, ownerId: nextOwnerId, periodStart, periodEnd, docType, note: note.trim() })
      .then((report) => {
        onDone(report);
        setOwnerId("");
        setPeriodStart("");
  ...
  return (
    <form className="report-form" onSubmit={submit}>
      ...
      <select value={docType} onChange={(event) => setDocType(event.target.value as ReportDocType)}>
        <option value="weekly_report">周报</option>
        <option value="summary">总结</option>
        <option value="other">其他</option>
      </select>
    ...
    <article className="item-card report-card">
      <div className="report-main">
        <div className="report-title">
          ...
          <span className={`report-status status-${report.status}`}>{statusLabels[report.status]}</span>
        </div>
        <p>
        ...
        <div className="report-files">
          {report.files.map((file) => (
            <a key={file.id} href={reportFileUrl(report.id, file.version)}>
              v{file.version} · {file.originalName}
            </a>
          ))}
        </div>
      </div>
      <div className="item-actions">
        {isOwner && report.status === "submitted" && (
          <>
            <button className="secondary" disabled={busy} onClick={() => run(() => approveReport(report.id))}>
              通过
            </button>
            <button
              className="secondary"
              disabled={busy}
              onClick={() => {
                const note = window.prompt("请填写退回原因");
                if (note && note.trim()) run(() => returnReport(report.id, note.trim()));
              }}
            >
              退回
            </button>
        ...
        {latest && (
          <a className="secondary" href={reportFileUrl(report.id, latest.version)}>
            下载
          </a>
        )}
      </div>
