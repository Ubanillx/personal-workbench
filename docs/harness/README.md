...
| 测试         | **全绿**：test:db 31 + test:webdav 13 + test:api 324（契约）+ test:auth 2 + test:ui 107（SSR 冒烟）= 477 项                                                                                                                                                          |
...
- **交付前四件套**：`npm run lint`（0 warning / 0 error）、`npm run typecheck`、`npm test`（477 项全绿）、`npm run build` 全绿，另跑 `npm run format:check` 与 `npx antd lint app`，并把结果写回本文状态快照。
