...
| 字段                                | 取值                                                                                                                                         | 约束位置                                                                                                                                                                                                                                                 |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tasks.status`                      | `todo` → `in_progress` → `pending_review` → `completed`                                                                                      | CHECK 约束；**不能通过 `PATCH` 直接改**，必须走 `progress`/`submit-review`/`approve`/`return` 接口。`pending_review` 只由「**执行者 ≠ 发布人**（`owner_id <> created_by`）的任务在 100% 时」产生，验收人 = 发布人（D-57，见 `ACCOUNTS_AND_ORGS.md` §25） |
