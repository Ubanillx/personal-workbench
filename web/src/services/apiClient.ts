...
export function access(token: string): Promise<{ user: import("../../../shared/types/domain").CurrentUser }> {
  return request<{ user: import("../../../shared/types/domain").CurrentUser }>("/auth/access", {
    method: "POST",
...
export const getTasks = (filters: { includeArchived?: boolean; status?: string; assignee?: string } = {}) =>
  request<import("../../../shared/types/domain").Task[]>(
    `/tasks?${new URLSearchParams({ ...(filters.includeArchived ? { includeArchived: "1" } : {}), ...(filters.status ? { status: filters.status } : {}), ...(filters.assignee ? { assignee: filters.assignee } : {}) }).toString()}`,
  );
export const createTask = (input: Record<string, unknown>) =>
  request<import("../../../shared/types/domain").Task>("/tasks", { method: "POST", body: JSON.stringify(input) });
export const updateTask = (id: string, input: Record<string, unknown>) =>
  request<import("../../../shared/types/domain").Task>(`/tasks/${id}`, { method: "PATCH", body: JSON.stringify(input) });
...
export const postTaskProgress = (id: string, input: { progress: number; note?: string }) =>
  request<import("../../../shared/types/domain").Task>(`/tasks/${id}/progress`, { method: "POST", body: JSON.stringify(input) });
export const submitTaskReview = (id: string, note?: string) =>
  request<import("../../../shared/types/domain").Task>(`/tasks/${id}/submit-review`, { method: "POST", body: JSON.stringify({ note }) });
export const approveTask = (id: string, note?: string) =>
  request<import("../../../shared/types/domain").Task>(`/tasks/${id}/approve`, { method: "POST", body: JSON.stringify({ note }) });
export const returnTask = (id: string, note: string) =>
  request<import("../../../shared/types/domain").Task>(`/tasks/${id}/return`, { method: "POST", body: JSON.stringify({ note }) });
...
export const addTaskComment = (id: string, content: string) =>
  request<ActivityItem>(`/tasks/${id}/comments`, { method: "POST", body: JSON.stringify({ content }) });
export const getTodos = () => request<import("../../../shared/types/domain").Todo[]>("/todos");
export const createTodo = (input: Record<string, unknown>) =>
  request<import("../../../shared/types/domain").Todo>("/todos", { method: "POST", body: JSON.stringify(input) });
export const updateTodo = (id: string, input: Record<string, unknown>) =>
  request<import("../../../shared/types/domain").Todo>(`/todos/${id}`, { method: "PATCH", body: JSON.stringify(input) });
...
export const createNote = (input: Record<string, unknown>) =>
  request<import("../../../shared/types/domain").Note>("/notes", { method: "POST", body: JSON.stringify(input) });
...
export const getFiles = (search = "", category = "") =>
  request<ImportantFile[]>(
    `/files?${new URLSearchParams({ ...(search ? { search } : {}), ...(category ? { category } : {}) }).toString()}`,
  );
export const createFile = (input: { name: string; filePath: string; category?: string }) =>
  request<ImportantFile>("/files", { method: "POST", body: JSON.stringify(input) });
...
export const createUser = (input: { name: string; role: "assistant" | "viewer" }) =>
  request<{ user: Collaborator; token: string }>("/users", { method: "POST", body: JSON.stringify(input) });
export const updateUser = (id: string, isActive: boolean) =>
  request<null>(`/users/${id}`, { method: "PATCH", body: JSON.stringify({ isActive }) });
...
export const previewInbox = (drafts: InboxDraft[]) =>
  request<InboxDraft[]>("/inbox/preview", { method: "POST", body: JSON.stringify({ drafts }) });
export const importInbox = (drafts: InboxDraft[]) =>
  request<{ created: import("../../../shared/types/domain").Task[]; skipped: Array<{ title: string; reason: string }> }>("/inbox/import", {
    method: "POST",
    body: JSON.stringify({ drafts }),
  });
export type ReviewData = {
  tasks: import("../../../shared/types/domain").Task[];
  summary: { total: number; completed: number; active: number; overdue: number; completionRate: number };
};
export const getReview = (filters: { from?: string; to?: string; ownerId?: string; status?: string } = {}) =>
  request<ReviewData>(
    `/review?${new URLSearchParams(Object.fromEntries(Object.entries(filters).filter(([, value]) => Boolean(value))) as Record<string, string>).toString()}`,
  );
...
export const markNotificationsRead = (input: { all?: boolean; taskId?: string; ids?: string[] }) =>
  request<null>("/notifications/read", { method: "POST", body: JSON.stringify(input) });
...
export const returnReport = (id: string, note: string) =>
  request<Report>(`/reports/${id}/return`, { method: "POST", body: JSON.stringify({ note }) });
