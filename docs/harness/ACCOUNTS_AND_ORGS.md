...
| D-55 | **重要文件按条选可见范围（2026-09-14）**：新建时选「给组织看 / 给自己看」（迁移 018 加 `visibility` + `owner_id`，**默认组织可见**，老数据零变化）；`private` = 只有**创建人**与**本组织的全局管理员**看得到，**可见即可改可删**，归属**不可转让**；组织隔离优先（跨组织一律 404）。删除随之改为**逐条判**：创建人可删自己的（含个人文件），组织管理者可删本组织的组织文件。逐条见 §24                                       |
| D-57 | **任务验收越权修复（2026-09-14）**：修复「执行者可以验收自己」的权限裂缝，并抽出前后端共用的 `canReviewTask()`；当前验收角色与 100% 状态口径由 D-58 进一步修正。逐条见 §25                                                                                                                                                                                                                                                   |
| D-58 | **任务验收流程修正（2026-09-14）**：组织管理者发布的任务由负责人进度到 `100%` 自动进入待验收，用户端不显示置灰的「提交验收」；发布任务的组织管理者负责通过 / 退回，全局管理员只作应急兜底，负责人永远不能验收自己负责的任务；`approve` / `return` 的服务端与页面共用 `canReviewTask()`。逐条见 §26                                                                                                                           |
...
| 重要文件：查看 / 新增 / 编辑 / 下载    | ✅ 全部（带筛选器）     | ✅ 本组织（**别人的个人文件除外**）      | ✅ 本组织自己的 + 组织可见的（D-54/D-55） |
| 重要文件：删除                         | ✅ 全部                 | ✅ 本组织的组织文件                      | ✅ **自己登记的**（含个人文件）（D-55）   |
...
8. **待办与随手记只属于归属人**（D-54）：连全局管理员都只能看自己的；跨组织 404，
   同组织但不是本人 403。
9. **个人文件只在「创建人 + 本组织全局管理员」之间可见**（D-55）：跨组织一律 404（即使是管理员），
   同组织但看不到时 403；这条与不变式 1（跨组织 404）**不冲突**——组织隔离永远优先。
10. **任务验收由发布任务的组织管理者执行**（D-58）：`approve` / `return` 要求当前账号是
    本组织 `manager` 且为任务发布人；全局 `admin` 只作应急兜底，但**任务的负责人永远不能验收自己负责的任务**。
    页面与服务端共用同一个判据（`canReviewTask`），不允许出现「按钮看不见、接口调得通」或反过来。
...
进度只能由负责人本人走 `POST /api/tasks/:id/progress`；组织管理者发布给普通成员的任务报到 100% 后自动进入待验收，
由发布人走 `approve` / `return` 完成验收。`submit-review` 端点仍保留用于兼容已有调用，页面不再展示
「提交验收」按钮。状态仍只能由 `progress` / `submit-review` / `approve` / `return` 推导，所以「对齐」的做法是把**当前值只读展示 + 把当前允许的流转平铺成按钮**，
...
### 23.4 重要文件：组织内全开放（可见范围由 D-55 细化，见 §24）
...
- 跨组织仍是 **404**，同组织但没权限是 **403** —— 前者不泄露存在性，后者对方本来就知道它存在。
- 不新增任何删除日志或操作审计（明确确认过）。
- 上面这张表在 **D-55 之后按「可见范围」细分**（个人文件对组织管理者不可见、创建人可删自己的），
  以 §24 为准；这里保留的是 D-54 那一步的口径。
...
- **待办 / 随手记的页面仍保留管理员的组织筛选器**：管理员只看得到自己的待办，所以他切组织
  ...
  「所属组织」字段一起删——那是另一轮改动。
- **`app/components/table-layout.ts` 不属于本次改动**（另一位并作者的表格排版工作，仓库里另有提交）。

## 24. 重要文件按条选可见范围（D-55，2026-09-14）

紧接 §23.4：D-54 把「重要文件」定成**组织内全开放**，但所有人共用一份列表，
个人用的报价底稿、客户原话、自己的备忘一登记就进了全组织的视图。
现在**每条索引自己带一个可见范围**，新建时选，之后随时可改。

### 24.1 取值与默认

| 取值                  | 谁能看到                                                 | 谁能改 / 删                                    |
| --------------------- | -------------------------------------------------------- | ---------------------------------------------- |
| `org`（**默认**）     | 本组织所有人（管理员跨组织）                             | 组织内所有人可改；删除限组织管理者与全局管理员 |
| `private`（给自己看） | **创建人** + **本组织的全局管理员**                      | 可见即可改可删；**归属不可转让**               |
| `NULL`（老数据兜底）  | 按 `org` 处理（迁移 018 已把存量行全部回填，行为零变化） | 同 `org`                                       |

- 新建的三个入口（「添加文件」抽屉、「选择文件」后保存、「浏览 WebDAV」的选中即登记 / 上传）
  都有这个选项，默认「给组织看」：不改变大多数人的习惯，要私密得主动选一下。
- 单选框用 `Radio.Group` 而不是 `Select`：只有两个取值，把「另一种选择是什么」摆在眼前。

### 24.2 关键取舍（都是明确确认过的）

| 项             | 结论                                                                              |
| -------------- | --------------------------------------------------------------------------------- |
| 管理员看得到吗 | **看得到**别人的个人文件，但**只在同一组织内**（跨组织仍 404，不变式 1 不受影响） |
| 管理员能改删吗 | **能**。管理员是应急通道，与「能看见就能管」一致                                  |
| 组织管理者     | **看不到也删不掉**别人的个人文件（403）；自己登记的个人文件照常管                 |
| 创建人         | 可以改自己的可见范围（双向：组织 ↔ 个人），也可以删自己的                         |
| 归属转让       | **不做**。要给别人就重建一条，免得再养一份候选名单与通知                          |
| 删除日志       | **不做**（与 §23.4 一致）                                                         |
...
| 位置                                      | 作用                                                                                                   |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `records.server.ts` `recordClauses()`     | 列表：组织条件 + `(visibility IS NULL OR visibility<>'private' OR owner_id=?)`（管理员不加后半个条件） |
| `records.server.ts` `assertFileAccess()`  | 单条：先组织边界（404），再可见范围（403「只能访问自己的个人文件」）                                   |
| `files.server.ts` `canDeleteFile()`       | 删除：管理员 / 创建人 / 「组织管理者且文件组织可见」三者之一                                           |
| `files.server.ts` `listFilesWithAccess()` | 页面逐条算 `deletableIds`，与 `deleteFileRecord` 共用同一个 `canDeleteFile`                            |

页面只渲染服务端算好的入口，不自己判角色；`owner_id` **不进 API 载荷**（那是归属信息），
但 `visibility` 与 `owned` 回传——前者是用户自己选的、后者是「我登记的」这个用户已知的事实。

### 24.4 验收证据

| 检查         | 结果                                                                                                                                               |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm test`   | 全绿：db 31 · 契约 **369/369 全部一致** · auth 2 · ui **128/128**                                                                                  |
| 迁移演练     | 在**正式库副本**上 11/11：三张表行数不变、重要文件全部有归属人、可见范围全部回填、**没有任何老数据被标成 private**（行为零变化）                   |
| 契约新增用例 | 默认可见范围、建个人文件、成员 / 管理者 / 管理员各视角的「看不看得见」、跨组织 404、组织↔个人互改、创建人自删、管理者删别人的个人文件 403 等 15 条 |
| 冒烟新增断言 | 成员自己的文件有删除入口、别人的没有；个人文件对同组织成员与组织管理者都不可见；管理者的列表里有自己的「我的报价底稿」                             |
| 静态门禁     | lint 0/0 · typecheck 四工程 0 · `format:check` 无漂移                                                                                              |

### 24.5 已知边界

- **个人文件的「下载」在未配置 WebDAV 时仍是 503**：下载走的是**当前账号**的 NAS 凭据，
  个人文件只改了索引的可见范围，不改变远端通道的账号归属。要用别人的个人文件，
  得让管理员在自己账号里配一次 WebDAV。
- **`visibility='private'` 的文件不进别人的概览展板**（`recordClauses` 同一条件），
  但会进**创建人自己**的「最近使用文件」卡片。
- **老数据的归属人是推断的**：迁移 018 与 017 同规则（本组织最早的启用组织管理者），
  正式库 2 条重要文件都认领到了组织管理者。若归属要改到别人名下，只能走 SQL（没有转移入口）。

## 25. 任务验收权归发布人（D-57 历史记录，2026-09-14）

> 本节记录 D-57 为修复执行者自验收而采取的中间方案；当前验收角色与页面流程已由 D-58
> 修正，现行规则见 §4、§26。

### 25.1 起因：执行者能验收自己

用户报障：「任务进展里进度到 100% 提交验收后，**任务执行者居然也能点『通过验收』**」。

查下来是一处真实的越权：`approveTask` / `returnTask` 当时**只判角色**（`canManageTasks` = admin/manager），
既不看「是不是这条任务的发布人」，也不看「是不是这条任务的负责人」。于是：

- 同组织里**任何一个**组织管理者都能验收别人的任务，不只是发布人；
- 只要执行者本人是组织管理者（或管理员），他就能**自己把任务验收通过**。

路由层 `requireManager` 只挡住了普通成员，所以「成员甲的越权」有契约用例盯着（403），
而「管理者的越权」一直没人看——**权限判据落在角色上，就会漏掉「角色够但关系不对」这一类**。

### 25.2 改前 → 改后

| 项                           | 改前                                                          | 改后（D-57）                                                                   |
| ---------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 谁能 `approve` / `return`    | 同组织的任何 admin / manager                                  | **任务的发布人**（`created_by`）；全局管理员保留兜底                           |
| 执行者（负责人）             | 若是管理者就能验收自己的任务                                  | **永远不能**（`owner_id = 我` 直接拒，`403`）                                  |
| `progress` 到 100%           | 按**执行者的角色**：member → 待验收；admin/manager → 直接完成 | 按**有没有第二方**：`created_by !== 我` → 待验收；`created_by = 我` → 直接完成 |
| 已完成任务的 `submit-review` | 允许（`progress=100` 就放行），能把它**拉回待验收**           | `400 INVALID_STATE`「已完成任务不能再提交验收」                                |
| 「待验收」通知收件人         | 负责人 + 本组织管理者                                         | 负责人 + **发布人** + 本组织管理者                                             |
| 页面按钮                     | `canManage && status === 'pending_review'` 就渲染             | `canReviewTask(me, task) && status === 'pending_review'`，判据与服务端同一份   |

### 25.3 判据落点

| 位置                                                              | 作用                                                                                              |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `app/lib/task-permissions.ts` `canReviewTask()`                   | **唯一判据**（纯函数，前后端共用）：非 admin/manager ⇒ 拒；负责人 ⇒ 拒；发布人或全局管理员 ⇒ 放行 |
| `task-service.server.ts` `approveTask` / `returnTask`             | `viewable()` 之后、状态判定之前插这道门，`403`「只有任务发布人可以验收 / 退回该任务」             |
| `task-service.server.ts` `reportProgress`                         | 100% 的落点改成 `createdBy === user.id ? 'completed' : 'pending_review'`                          |
| `task-service.server.ts` `submitReview`                           | 已完成任务再提交验收 ⇒ `400`                                                                      |
| `tasks.server.ts` `canManageTasks()`                              | 改为委托 `canManageRole()`，注释里写明**验收不在这条线上**（免得下一个人顺手把验收接到这里）      |
| `tasks.server.ts` `notifyParticipants()` / `notifyOverdueTasks()` | 非私密任务的收件人补上 `createdBy`（发布人可能是不属于该组织的管理员）                            |
| `routes/tasks.tsx`（列表行内动作 + `TaskProgressPanel`）          | 用同一个 `canReviewTask` 算 `canReview`，不成立就**整块不渲染**（沿用 D-54 的做法）               |

`task-permissions.ts` 是普通 `.ts`（不是 `.server.ts`）：页面要在渲染按钮**之前**算出「我能不能验收」，
判据必须能进浏览器包；它只有入参和返回、不碰数据库，所以能同时被服务端与页面 import，
不会出现「按钮看得见、接口调不通」这种双向漂移。

### 25.4 为什么全局管理员要兜底

发布人可能停用、离职、被移出组织，而任务卡在 `pending_review`：没有兜底就**没有人能把它推完**。
管理员是全局角色（D-26，不隶属任何组织），沿用 §24.2「管理员是应急通道」的同一口径。
兜底只放宽「发布人」这一条，**不放宽「负责人」**——管理员的活也由他自己交、别人验收。

### 25.5 顺带堵掉的两条

- **`submit-review` 能把已完成任务拉回待验收**：100% 之后 `reportProgress` 已经定局，但
  `submit-review` 只校验「进度 = 100」，于是负责人可以「完成 → 待验收 → 再做一遍」。
  用户看到的那条越权路径正是这样走出来的（管理者负责的任务到 100% 直接完成，他再从列表里点了「提交验收」）。
- **列表与详情不一致的「提交验收」按钮**：详情抽屉对 `completed` 早已不渲染该按钮，列表却只禁用了
  `pending_review` 与 `progress < 100`，于是同一个动作在一处能点、一处不能点。现在两处口径一致（服务端也拒绝）。

### 25.6 契约与测试

新增 / 改名的用例（`tools/contract/cases.ts`，golden 已重录）：

| 用例                                     | 钉住的事                                                   |
| ---------------------------------------- | ---------------------------------------------------------- |
| `tasks.create.admin.for-manager`         | 造一条「管理员发布、组织管理者负责」的任务                 |
| `tasks.progress.managerA.executor`       | 负责人是管理者时，100% 也要进待验收（旧口径直接完成）      |
| `tasks.approve.managerA.executor`        | **执行者验收自己负责的任务 ⇒ 403**（本次修复的核心）       |
| `tasks.return.managerA.executor`         | 退回是同一个动作的另一面 ⇒ 403                             |
| `tasks.approve.admin`                    | 发布人（管理员）兜底验收 ⇒ 200                             |
| `tasks.submit-review.managerA.completed` | 已完成任务不能再提交验收 ⇒ 400                             |
| `tasks.progress.memberA.own-task`        | 自己发布给自己做的任务 100% ⇒ 直接完成，不进待验收         |
| `tasks.approve.managerA.not-publisher`   | 同组织管理者但不是发布人 ⇒ 403（**这条曾经是 200**）       |
| `tasks.approve.managerA`                 | 改指夹具里 managerA 发布的那条待验收任务：发布人验收 ⇒ 200 |

`tasks.approve.admin.publisher` 顺便把「发布人通过」的正向路径也钉住了（`taskCreated` 的终态与改动前一致，
下游用例不受影响）。`test:ui`（SSR 冒烟）没有验收相关的断言，因此本次不动冒烟用例。

验收证据：

| 检查       | 结果                                                                                                  |
| ---------- | ----------------------------------------------------------------------------------------------------- |
| `npm test` | 全绿：db 31 · webdav 13 · 契约 **378/378**（golden 已重录）· auth 2 · ui **138/138**                  |
| 静态门禁   | lint 0/0（163 文件）· typecheck 四工程 0 · `npx antd lint app` 119 文件无问题 · `format:check` 无漂移 |

### 25.7 没做的（明确的取舍）

- **不改 `notifyParticipants` 里「组织管理者全都收到」这条**：组织管理者仍然收得到本组织任务的动态
  （他们看得见、也能评论/改元信息），只是**不能验收**；要精确到「只发验收人」得给通知加一层收件人推导，
  收益不大（多出来的只是通知条数，不是权限）。
- **不给「谁验收」做配置项**：发布人 = 验收人，是这次确认下来的口径；要做「指定验收人」得先有字段与界面，
  属于另一轮改动。
- **`/api/review`（验收视图）仍限 admin/manager 只读**：它是聚合读取，不改变任何状态；
  页面上的验收入口已经按 `canReviewTask` 收口。

## 26. 任务验收流程修正（D-58，2026-09-14）

### 26.1 用户侧不再提交验收

D-57 上线后，任务进度的页面仍给负责人渲染「提交验收」：列表在任务已经是
`pending_review` 或 `completed` 时只是把按钮置灰，详情与列表的表现也不一致。
同时，成员自己创建并负责的任务在 100% 时被直接标成 `completed`，组织管理者没有验收入口。

当前口径改为：

- 组织管理者发布给普通成员的任务，负责人把进度保存为 100% 后，服务端自动将任务置为 `pending_review`；页面不再渲染「提交验收」按钮。
- 发布任务的本组织组织管理者可以对待验收任务执行「通过验收」或「退回修改」，全局管理员只作应急兜底。
- 负责人不能验收自己负责的任务；管理者作为执行者被他人指派时由任务发布人处理。
- `canReviewTask()` 仍是页面与服务端共用的唯一判据；`submit-review` API 保留用于兼容已有调用。

### 26.2 回归覆盖

契约覆盖「组织管理者发布、成员完成后由发布人验收」以及「管理者作为负责人不能自验收」两条路径；
SSR 冒烟同时确认成员页面不出现「提交验收」，组织管理者页面出现「通过验收」。
