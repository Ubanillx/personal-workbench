...
test("周报功能会执行上传、代传、审核、退回、版本与权限", async (t) => {
  ...
  const app = await buildApp({
    config: loadConfig({ NODE_ENV: "test", DATABASE_PATH: databasePath, UPLOADS_DIR: uploadsDir }, process.cwd()),
    database,
    uploadsDir,
  });
  t.after(async () => {
  ...
  assert.ok(
    ownerNotifs
      .json()
      .data.some((n: { reportId: string; eventType: string }) => n.reportId === reportId && n.eventType === "report_submitted"),
  );
  ...
  // 助理收到“周报已退回”通知
  assert.ok(
    (await request(app, assistant, "GET", "/api/notifications?unread=1"))
      .json()
      .data.some((n: { reportId: string; eventType: string }) => n.reportId === reportId && n.eventType === "report_returned"),
  );
  ...
  assert.ok(
    (await request(app, assistant, "GET", "/api/notifications?unread=1"))
      .json()
      .data.some((n: { reportId: string; eventType: string }) => n.reportId === reportId && n.eventType === "report_approved"),
  );
...
function requestRaw(app: Awaited<ReturnType<typeof buildApp>>, cookie: string, method: "GET", url: string) {
  return app.inject({ method, url, headers: { cookie } });
}
...
function upload(
  ...
  cookie: string,
  url: string,
  payload: { fields: Record<string, string>; file: { name: string; content: Buffer } },
) {
