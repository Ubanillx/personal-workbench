...
| 测试         | **全绿**：test:db 31 + test:webdav 13 + test:api 378（契约）+ test:auth 2 + test:ui 138（SSR 冒烟）= 562 项                                                                                                                                                          |
| 类型检查     | 通过（TS 7.0.2 原生 tsc；server + tools + app + test 四个工程，strict + noUncheckedIndexedAccess）                                                                                                                                                                   |
| Lint         | **0 warning / 0 error**（oxlint 1.82，163 文件）                                                                                                                                                                                                                     |
...
| 契约快照     | **378 条用例 / 128 个 method+path 组合 / 17 个域**，覆盖九种身份（含跨组织隔离、webdav.* 与 files.download.* 用例）；`contract:compare` 全绿                                                                                                                         |
| 前端 UI      | **antd v6.6.3** + icons 6.3.4；`antd doctor` 14 项全过，`antd lint app` 119 文件无问题                                                                                                                                                                               |
| 全栈迁移     | **Phase 0–4 全部完成** → `FULLSTACK_MIGRATION.md`                                                                                                                                                                                                                    |
| 账号与组织   | **A–E 全部完成**（正式库已迁移至 011，契约 324/324）→ `ACCOUNTS_AND_ORGS.md`                                                                                                                                                                                         |
| 周报存储     | **D-46 已实现，配置口径按 D-52 + D-53 收敛**：正文只写 NAS（`<本组织上传根>/<用户名>/<起止日期>/<文件名>`），归属人=登录账号，管理员不提交；上传**共用管理员那份 WebDAV 连接**，**上传目录按组织配**（组织管理者改本组织，默认用连接的浏览根） → `REPORTS_WEBDAV.md` |
...
| 契约快照录制     | `npm run contract:capture`                                               | 黑盒跑 378 条用例，写入 `test/contract/golden/`（**先 `npm run build`**；自动拉起假 WebDAV）                                                  |
...
| 单项测试         | `npm run test:db` / `test:webdav` / `test:api` / `test:auth` / `test:ui` | 数据层 / WebDAV 客户端（假服务器）/ 378 条契约回放 / 注册登录改密端到端 / SSR 冒烟                                                            |
| 生成夹具库       | `npm run contract:fixture`                                               | 输出可直接用作 `DATABASE_PATH` 的临时库（含固定令牌与管理员 WebDAV 连接行）                                                                   |
...
- **交付前四件套**：`npm run lint`（0 warning / 0 error）、`npm run typecheck`、`npm test`（562 项全绿）、`npm run build` 全绿，另跑 `npm run format:check` 与 `npx antd lint app`，并把结果写回本文状态快照。
- **契约门禁（长期有效）**：任何改动端点或数据层的提交，都必须让 `npm run contract:compare` 保持 **378** 条 100% 一致；不一致时不允许提交，除非是**有意**的行为变更并同步重新 capture。
...
- **不要直接读 `.env` 里的地址做判断**：WebDAV 地址是部署级的（`WEBDAV_URL`）；凭据只有一份「按账号」的
  `webdav_settings`（重要文件），**周报上传共用管理员那一份**（D-52），别再去找第二份凭据表。
  `report_upload_settings` 里现在只有「上传根目录」，且**没有这一行 = 用默认目录**（跟随连接的浏览根）。
