...
export function InboxPage(): React.ReactElement {
  ...
  return (
    <section className="page-section">
      <div className="eyebrow">WECHAT WORK</div>
      <h1>企微收件箱</h1>
      <p className="page-subtitle">粘贴聊天记录，逐条编辑标题、负责人、优先级与截止日期。系统会标记疑似重复任务。</p>
      <textarea
      ...
      <div className="toolbar">
        <button disabled={!raw.trim() || busy} onClick={parse}>
          {busy ? "处理中…" : "解析消息"}
        </button>
        {drafts.length > 0 && (
          <button className="secondary" disabled={busy} onClick={submit}>
            导入选中任务
          </button>
        )}
      </div>
      ...
      <div className="collection">
        {drafts.map((draft) => (
          <article className={`item-card inbox-draft ${draft.duplicate ? "duplicate-card" : ""}`} key={draft.id}>
            <label className="check-row">
              <input type="checkbox" checked={draft.selected} onChange={(e) => change(draft.id, { selected: e.target.checked })} />
              <input className="inline-input" value={draft.title} onChange={(e) => change(draft.id, { title: e.target.value })} />
            </label>
            {draft.duplicate && <span className="warning-tag">疑似重复</span>}
            <div className="inbox-fields">
              <select value={draft.ownerId} onChange={(e) => change(draft.id, { ownerId: e.target.value })}>
              ...
              <select value={draft.priority} onChange={(e) => change(draft.id, { priority: e.target.value as Draft["priority"] })}>
                <option>P0</option>
                <option>P1</option>
                <option>P2</option>
              </select>
              <input type="date" value={draft.dueDate ?? ""} onChange={(e) => change(draft.id, { dueDate: e.target.value || null })} />
            </div>
            <small className="muted">发送人：{draft.sender || "未识别"}</small>
          </article>
        ))}
      </div>
    </section>
...
export function FilesPage(): React.ReactElement {
  ...
  return (
    <section className="page-section">
      ...
      <h1>重要文件</h1>
      <p className="page-subtitle">只保存本机或共享盘路径索引，不上传文件内容。</p>
      <form
        className="create-row file-form"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="文件名称" />
        ...
        <button disabled={busy}>{busy ? "保存中…" : "添加"}</button>
      </form>
      <div className="toolbar">
          ...
          files.map((file) => (
            <article className="item-card" key={file.id}>
              <div>
                ...
                <small className="muted">最近使用：{file.lastUsedAt ? new Date(file.lastUsedAt).toLocaleString() : "尚未记录"}</small>
              </div>
              <div className="item-actions">
                <button className="secondary" onClick={() => copy(file)}>
                  复制路径
                </button>
                <button
                  className="icon-button danger"
                  onClick={() => {
                    if (window.confirm("确定删除文件索引吗？"))
...
export function CollaborationPage(): React.ReactElement {
  ...
  if (accessLoading)
    return (
      <section className="page-section">
        <div className="loading-inline">正在检查协作管理权限…</div>
      </section>
    );
  ...
  if (!currentUser || currentUser.role !== "owner") {
    return (
      <section className="page-section">
        ...
        <div className="error-panel">
          <div>
            <strong>当前账户没有成员管理权限</strong>
            <p>{error || "请退出当前账户，并使用主人令牌重新登录。"}</p>
          </div>
          <button className="secondary" disabled={busy} onClick={logoutForOwner}>
            退出并重新登录
          </button>
        </div>
      </section>
  ...
  return (
    <section className="page-section">
      <div className="eyebrow">COLLABORATION</div>
      <h1>协作管理</h1>
      <p className="page-subtitle">停用成员会立即让会话和长期令牌失效；启用后请重新生成令牌。</p>
      {error && (
        <div className="error-panel">
          {error}
          <button className="secondary" onClick={load}>
      ...
      <form className="create-row" onSubmit={create}>
        ...
        <select value={role} onChange={(event) => setRole(event.target.value as "assistant" | "viewer")}>
          <option value="assistant">助理</option>
          <option value="viewer">查看者</option>
        </select>
        <button disabled={!name.trim() || busy}>{busy ? "创建中…" : "新增成员"}</button>
      </form>
      <div className="access-address">
        <strong>助理访问地址</strong>
        {accessInfo?.lanUrls.length ? (
          <div className="lan-address-list">
            {accessInfo.lanUrls.map((url) => (
              <div className="copy-row" key={url}>
                <code>{url}</code>
                <button className="secondary" onClick={() => copy(url, "访问地址")}>
                  复制
                </button>
              </div>
            ))}
          ...
          <p className="muted">
            未检测到局域网地址，请确认正式服务使用 <code>HOST=0.0.0.0</code> 启动。
          </p>
        )}
        <p>{accessInfo?.warning ?? "仅允许同一局域网的助理访问；不要发送 127.0.0.1、0.0.0.0 或带令牌的链接。"}</p>
      </div>
      {token && (
        <div className="token-panel">
          <strong>长期令牌（仅显示一次）</strong>
          <code>{token}</code>
          <div className="item-actions">
            <button className="secondary" onClick={() => copy(token, "长期令牌")}>
              复制令牌
            </button>
        ...
        {users.map((user) => (
          <article className="item-card" key={user.id}>
            <div>
              ...
              <p>
                {roleLabel[user.role]} · {user.isActive ? "启用中" : "已停用"}
              </p>
            </div>
            {user.role !== "owner" && (
              <div className="item-actions">
                <button className="secondary" disabled={busy} onClick={() => toggleMember(user)}>
                  {user.isActive ? "停用" : "启用"}
                </button>
                <button className="secondary" disabled={busy || !user.isActive} onClick={() => rotateToken(user)}>
                  重发令牌
                </button>
              </div>
            )}
        ...
        <>
          <div className="dashboard-grid">
            <div className="stat-card">
              <strong>{data.summary.completionRate}%</strong>
              <span>完成率</span>
            </div>
            <div className="stat-card">
            ...
            <div className="stat-card">
              <strong>{data.tasks.filter((task) => task.status === "pending_review").length}</strong>
              <span>待验收</span>
            </div>
          </div>
            ...
            {data.tasks.length ? (
              data.tasks.map((task) => (
                <div className="data-row" key={task.id}>
                  ...
                  <small>
                    {task.ownerName ?? "未分配"} · {labelsFor(task.status)} · {task.progress}%
                  </small>
                </div>
              ))
            ) : (
              <p className="muted">当前条件下没有任务。</p>
            )}
          </div>
        </>
      )}
...
export function AccessPage({ onLogout }: { onLogout: () => void }): React.ReactElement {
  return (
    <section className="page-section">
      ...
      <h1>访问验证</h1>
      <p className="page-subtitle">当前浏览器已通过 Cookie 会话验证。</p>
      <button className="secondary" onClick={() => void logout().then(onLogout)}>
        退出当前账户
      </button>
    </section>
  );
