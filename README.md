...
- 列表页（任务 / 待办 / 随手记 / 周报 / 重要文件与设置页的成员·组织·账号三张表）一律**服务端分页**：`?page=` / `?size=` / `?sort=` / `?order=` 是唯一真相，筛选、排序、`COUNT(*)` 与 `LIMIT/OFFSET` 都在 SQL 里做，loader 只回当前一页（`app/lib/paging.ts` + `app/lib/paging.server.ts`）。
...
WebDAV 在这个项目里服务**两件事**，但只需要配**一份连接**（D-52）：

| 用途                        | 用哪个连接                            | 还要配什么                                                                                           |
| --------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **「重要文件」浏览 / 上传** | 每个账号自己（各人用自己的 NAS 账号） | `/settings` →「WebDAV」Tab 上半部分（按账号存库，保存即时生效）                                      |
| **周报/总结正文的存储**     | **共用管理员那份连接**（D-52）        | `/settings` →「WebDAV」Tab 的「周报上传」卡片里选**本组织**的上传根目录（D-53，组织管理者 / 管理员） |

两份共用同一个**部署级地址**：`.env` 里的 `WEBDAV_URL`（改完要**重启服务**）。
周报那边**不再单独填账号密码**——它就是管理员在「WebDAV 连接」里保存的那一份；
**不同组织写进 NAS 上不同的目录**，各组织的目录由该组织的管理者自己挑。
...
- **连接共用管理员那份**（D-52）：周报上传/下载都用管理员在「WebDAV 连接」里保存的账号，
  成员不需要、也进不去设置页；
- **上传根目录按组织配**（D-53）：组织管理者在 `/settings → WebDAV → 周报上传` 里挑本组织的目录，
  管理员可以在卡里切换组织；**不同组织写进 NAS 上不同的目录**；
- 落点：`<本组织的上传根目录>/<登录用户名>/<起止日期>/<原文件名>`；没挑目录就用那份连接的浏览根目录，
  例如连接根是 `/` 就是 `/zhangsan/2026-09-01_2026-09-07/第八周周报.docx`，阿尔法组挑了 `/阿尔法` 就是
  `/阿尔法/zhangsan/2026-09-01_2026-09-07/第八周周报.docx`；
- 退回后重传加 `_v2` / `_v3` 后缀；同人同期同名再交一份自动加 `_2`；**远端已有文件绝不被覆盖**；
- 下载走**服务端流式代理**（浏览器 → 应用 → NAS），登录与组织隔离照旧生效，NAS 凭据不出服务器。

第一次启用：管理员先在 `/settings → WebDAV` 的「WebDAV 连接」里保存一次连接（周报就用它）；
随后各组织的管理者在下面「周报上传」卡里点「选择目录」挑本组织的目录（不挑就用连接的浏览根目录）→ 保存，
想回到默认随时点「恢复默认」。**还没有任何管理员保存过连接**时，成员提交周报会拿到
「周报存储未配置：请联系管理员…」。
...
3. **配了地址不会自动对所有人开放「重要文件」的远端通道**：每个人都要在设置页里保存一次自己的账号（NAS 允许匿名访问就留空保存）——
   这样没在用 WebDAV 的人不会被远端探测拖慢，也不会看到「远端不可达」的告警。**周报上传不受这条影响**：它共用管理员那份连接，成员不用配任何东西。

完整说明（结构、边界、取舍、排障）见 [`docs/harness/WEBDAV.md`](docs/harness/WEBDAV.md)。

> NAS 层的可见性：周报写入的是一个共享目录，能打开该共享的人可以看到所有人的周报。
...
| 脚本          | 内容                                                                                                                                                                            | 当前基线       |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `test:db`     | SQLite 备份 / JSON 迁移 / 迁移前备份 / 账号密码 / 自举 / **周报远端存储与搬迁 CLI**                                                                                             | 31 项          |
| `test:webdav` | WebDAV 客户端（本地假服务器）：PROPFIND 解析 / 路径越界 / 上传补建目录 / 条件上传 / 流式下载                                                                                    | 13 项          |
| `test:api`    | 369 条 HTTP 契约用例，逐条对比 `test/contract/golden/contract.golden.json`                                                                                                      | 369 条全部一致 |
| `test:auth`   | 端到端：注册 → 登录 → 强制改密门禁 → 改密 → 业务端点恢复                                                                                                                        | 2 项           |
| `test:ui`     | SSR 冒烟：登录 → 外壳 → 各页面渲染出真实数据 + 权限边界（跨组织回弹 / 私密任务 / 本人数据 / 文件删除入口）+ 服务端分页（跨页不重不漏 / 越界钳末页 / 条数白名单 / 表头排序生效） | 138 项检查     |

改动**权限**相关代码前，先读 [`docs/harness/ACCOUNTS_AND_ORGS.md`](docs/harness/ACCOUNTS_AND_ORGS.md) §4 的权限矩阵与 §23
（D-54：私密任务的三个可见方、待办/随手记的本人数据、重要文件的删除门槛）。
一条硬约束：**列表里看不见的，接口里也必须改不动**——读写共用同一个可见性判据。
