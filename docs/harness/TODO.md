...
| MIG-02  | Phase 1 数据/安全层搬迁           | 新工程用临时库启动，`/api/health`、`/api/auth/*` 通过 golden 比对                                                          | 待办                 |
| MIG-03  | Phase 2 迁移 48 端点              | `contract:compare` 对新实现 100% 一致；Fastify 退役；四件套全绿                                                            | 待办                 |
| MIG-04  | Phase 3 前端迁移与测试重写        | 8 个页面功能等价（逐页走查）；测试重写完成（顺带解决 `DEBT-14`）                                                           | 待办                 |
