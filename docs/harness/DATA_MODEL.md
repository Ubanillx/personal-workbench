...
| 字段                                | 取值                                                                                                                                         | 约束位置                                                                                                                                                                                                                                           |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tasks.status`                      | `todo` → `in_progress` → `pending_review` → `completed`                                                                                      | CHECK 约束；**不能通过 `PATCH` 直接改**，必须走 `progress`/`submit-review`/`approve`/`return` 接口。组织管理者发布给成员的任务在 100% 时进入 `pending_review`，由发布人验收；自己发布并自己执行时可直接完成（D-58，见 `ACCOUNTS_AND_ORGS.md` §26） |
