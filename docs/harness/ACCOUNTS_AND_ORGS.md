...
| D-57 | **任务验收权归发布人（2026-09-14）**：`approve` / `return` 从「同组织任何管理者」收敛为「**任务的发布人**（`created_by`）」——执行者（负责人）即便是管理者也不能验收自己负责的任务；`100%` 之后走不走验收改判「**有没有第二方**」（`created_by !== 我` ⇒ 待验收），已完成任务不能再 `submit-review` 拉回待验收；待验收通知补上发布人（管理员发布的组织任务原本没人收得到）。逐条见 §25                                        |
...
10. **任务验收权只属于发布人**（D-57）：`approve` / `return` 要求「当前账号 = `tasks.created_by`」
    （全局管理员可兜底），**任务的负责人永远不能验收自己负责的任务**，哪怕他是组织管理者。
