...
| 测试         | **246 项全绿**：test:db 16 + test:api 305（契约）+ test:auth 2 + test:ui 72（SSR 冒烟）                             |
...
| 契约快照     | **305 条用例**覆盖约 62 个端点 × 九种身份（含跨组织隔离用例）；`contract:compare` 全绿                              |
...
| 契约快照录制     | `npm run contract:capture`                               | 黑盒跑 305 条用例，写入 `test/contract/golden/`（**先 `npm run build`**）             |
...
| 单项测试         | `npm run test:db` / `test:api` / `test:auth` / `test:ui` | 数据层 / 305 条契约回放 / 注册登录改密端到端 / SSR 冒烟                               |
...
- **契约门禁（长期有效）**：任何改动端点或数据层的提交，都必须让 `npm run contract:compare` 保持 305 条 100% 一致；不一致时不允许提交，除非是**有意**的行为变更并同步重新 capture。
...
- 自动化：数据库、认证、API 契约和页面冒烟测试通过；页面冒烟共 74 项。
