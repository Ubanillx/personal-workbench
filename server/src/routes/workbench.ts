...
export async function registerWorkbenchRoutes(app: FastifyInstance, options: Options): Promise<void> {
  ...
  const require = (request: FastifyRequest, reply: FastifyReply, _database: Db, _cookie: string): User | null =>
    requireAuth(request, reply, db, cookie);
  app.post("/api/auth/access", async (request, reply) => {
    const token = String(((request.body ?? {}) as Record<string, unknown>).token ?? "").trim();
    const user = token ? userByToken(db, token) : null;
    if (!user) return reply.code(401).send(fail("INVALID_TOKEN", "访问令牌无效"));
    const raw = session(db, user.id);
    reply.setCookie(cookie, raw, { httpOnly: true, sameSite: "lax", secure: request.protocol === "https", path: "/", maxAge: 2592000 });
    return reply.send(ok({ user }));
  });
  app.post("/api/auth/logout", async (request, reply) => {
    const raw = request.cookies[cookie];
    if (raw) run(db, "UPDATE access_sessions SET revoked_at=? WHERE session_hash=?", now(), hash(raw));
    reply.clearCookie(cookie, { path: "/" });
    return reply.send(ok(null));
  });
  app.get("/api/auth/me", async (request, reply) => {
    const user = auth(request, db, cookie);
    if (!user) return reply.code(401).send(fail("UNAUTHENTICATED", "请先完成访问验证"));
    return reply.send(ok({ user }));
  });
  app.get("/api/access-info", async (request, reply) => {
    ...
    return reply.send(
      ok({
        ...
        localUrl: `http://127.0.0.1:${options.config.port}`,
        lanUrls: addresses.map((ip) => `http://${ip}:${options.config.port}`),
  ...
  app.get("/api/dashboard", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    notifyOverdueTasks(db);
    const tasks = visible(db, user, false);
    const todos =
      user.role === "owner"
        ? rows(
            db,
    ...
    return reply.send(
      ok(
        visible(
          db,
          user,
          user.role === "owner" && q.includeArchived === "1",
          typeof q.status === "string" ? q.status : undefined,
          typeof q.assignee === "string" ? q.assignee : undefined,
        ),
      ),
    );
  });
  app.post("/api/tasks", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    if (user.role === "viewer") return reply.code(403).send(fail("FORBIDDEN", "查看者不能创建任务"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    const title = String(b.title ?? "").trim();
    if (!title) return reply.code(400).send(fail("VALIDATION_ERROR", "任务标题不能为空"));
    const ownerId = user.role === "assistant" ? user.id : ownerIdOf(b.ownerId, user.id);
    const owner = validateOwner(db, ownerId);
    ...
    run(
      ...
      user.role === "owner" && b.isPrivate ? 1 : 0,
      stamp,
      stamp,
      null,
      null,
    ...
    event(db, id, user, "task_created", owner.user && owner.user.id !== user.id ? `创建并分配给 ${owner.user.name}` : "创建任务");
    if (owner.user?.role === "assistant" && owner.user.id !== user.id)
      notify(db, [owner.user.id], user.id, id, "task_assigned", "收到新任务", `你收到来自 ${user.name} 的任务“${title}”`);
    return reply.code(201).send(ok(findTask(db, id)));
  });
  app.patch("/api/tasks/:id", async (request, reply) => {
    ...
    if (!current) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (current.archivedAt) return reply.code(400).send(fail("ARCHIVED", "请先恢复归档任务"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    if (b.progress !== undefined || b.status !== undefined)
      return reply.code(400).send(fail("FIELD_FORBIDDEN", "请使用进度和验收接口变更任务状态"));
    const title = String(b.title ?? current.title).trim();
    if (!title) return reply.code(400).send(fail("VALIDATION_ERROR", "任务标题不能为空"));
    const ownerId = Object.prototype.hasOwnProperty.call(b, "ownerId") ? ownerIdOf(b.ownerId, null) : current.ownerId;
    const owner = validateOwner(db, ownerId);
    if (!owner.valid) return reply.code(400).send(fail("VALIDATION_ERROR", owner.message));
    const isPrivate = b.isPrivate === undefined ? Number(current.isPrivate) : b.isPrivate ? 1 : 0;
    if (isPrivate && ownerId !== "owner") return reply.code(400).send(fail("VALIDATION_ERROR", "私密任务只能由主人负责"));
    const stamp = now();
    run(
      db,
      "UPDATE tasks SET title=?,description=?,priority=?,due_date=?,owner_id=?,is_private=?,updated_at=? WHERE id=?",
      title.slice(0, 240),
      String(b.description ?? current.description),
      priority(b.priority ?? current.priority),
      b.dueDate === undefined ? current.dueDate : date(b.dueDate),
      ownerId,
      isPrivate,
      stamp,
      id,
    ...
    if (ownerId !== current.ownerId) {
      event(db, id, user, "task_reassigned", `负责人从 ${current.ownerName ?? "未分配"} 改为 ${owner.user?.name ?? "未分配"}`);
      notify(
        db,
    ...
    return reply.send(ok(findTask(db, id)));
  });
  app.post("/api/tasks/:id/progress", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    const id = idOf(request);
    const current = findTask(db, id);
    if (!current) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (current.archivedAt) return reply.code(400).send(fail("ARCHIVED", "请先恢复归档任务"));
    if (user.role === "viewer" || current.ownerId !== user.id) return reply.code(403).send(fail("FORBIDDEN", "只能更新自己负责的任务"));
    if (current.status === "pending_review") return reply.code(400).send(fail("INVALID_STATE", "任务已提交验收，请等待主人处理"));
    if (current.status === "completed") return reply.code(400).send(fail("INVALID_STATE", "已完成任务不能再更新进度"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    ...
    log(db, id, user, String(b.note ?? b.log ?? `进度更新为 ${p}%`).trim(), p);
    const updated = findTask(db, id);
    if (status === "pending_review") event(db, id, user, "task_submitted", "进度达到 100%，提交主人验收");
    notifyParticipants(
      db,
      updated,
      user.id,
  ...
  app.post("/api/tasks/:id/submit-review", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    const id = idOf(request);
    const task = findTask(db, id);
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (task.archivedAt) return reply.code(400).send(fail("ARCHIVED", "请先恢复归档任务"));
    if (user.role !== "assistant" || task.ownerId !== user.id) return reply.code(403).send(fail("FORBIDDEN", "只有任务助理可以提交验收"));
    if (task.status === "pending_review") return reply.send(ok(task));
    if (task.progress < 100) return reply.code(400).send(fail("VALIDATION_ERROR", "进度达到100%后才能提交验收"));
    run(db, "UPDATE tasks SET status='pending_review',updated_at=? WHERE id=?", now(), id);
    event(db, id, user, "task_submitted", String(((request.body ?? {}) as Record<string, unknown>).note ?? "提交主人验收"));
    const updated = findTask(db, id);
    notifyParticipants(db, updated, user.id, "task_submitted", "任务待验收", `任务“${updated.title}”等待主人验收`);
    return reply.send(ok(updated));
  });
  app.post("/api/tasks/:id/approve", async (request, reply) => {
    ...
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (task.archivedAt) return reply.code(400).send(fail("ARCHIVED", "请先恢复归档任务"));
    if (task.status !== "pending_review") return reply.code(400).send(fail("INVALID_STATE", "只有待验收任务可以通过"));
    const stamp = now();
    run(db, "UPDATE tasks SET status='completed',progress=100,completed_at=?,updated_at=? WHERE id=?", stamp, stamp, id);
    event(db, id, user, "task_approved", String(((request.body ?? {}) as Record<string, unknown>).note ?? "主人验收通过"));
    const updated = findTask(db, id);
    notifyParticipants(db, updated, user.id, "task_approved", "任务验收通过", `任务“${updated.title}”已通过验收`);
    return reply.send(ok(updated));
  });
  app.post("/api/tasks/:id/return", async (request, reply) => {
    ...
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (task.archivedAt) return reply.code(400).send(fail("ARCHIVED", "请先恢复归档任务"));
    if (task.status !== "pending_review") return reply.code(400).send(fail("INVALID_STATE", "只有待验收任务可以退回"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    const p = Math.min(task.progress, 99);
    const note = String(b.note ?? "主人退回任务，请继续处理").trim() || "主人退回任务，请继续处理";
    run(db, "UPDATE tasks SET status='in_progress',progress=?,completed_at=NULL,updated_at=? WHERE id=?", p, now(), id);
    event(db, id, user, "task_returned", note);
    const updated = findTask(db, id);
    notifyParticipants(db, updated, user.id, "task_returned", "任务已退回", `任务“${updated.title}”已退回`);
    return reply.send(ok(updated));
  });
  app.post("/api/tasks/:id/archive", async (request, reply) => {
    ...
    if (!task.archivedAt) {
      const stamp = now();
      run(db, "UPDATE tasks SET archived_at=?,updated_at=? WHERE id=?", stamp, stamp, id);
      event(db, id, user, "task_archived", "任务已归档");
    }
    return reply.send(ok(null));
  });
  app.post("/api/tasks/:id/restore", async (request, reply) => {
    ...
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (task.archivedAt) {
      run(db, "UPDATE tasks SET archived_at=NULL,updated_at=? WHERE id=?", now(), id);
      event(db, id, user, "task_restored", "任务已恢复");
    }
    return reply.send(ok(findTask(db, id)));
  });
  app.delete("/api/tasks/:id", async (request, reply) => {
    ...
    const task = findTask(db, id);
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (!task.archivedAt) return reply.code(400).send(fail("ARCHIVE_REQUIRED", "请先归档任务，再彻底删除"));
  ...
  app.get("/api/tasks/:id/comments", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    const task = findTask(db, idOf(request));
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (!canView(task, user)) return reply.code(403).send(fail("FORBIDDEN", "无权查看该任务"));
    return reply.send(
      ok(
        rows(
          db,
  ...
  app.post("/api/tasks/:id/comments", async (request, reply) => {
    ...
    const task = findTask(db, idOf(request));
    if (!task) return reply.code(404).send(fail("NOT_FOUND", "任务不存在"));
    if (!canView(task, user)) return reply.code(403).send(fail("FORBIDDEN", "无权评论该任务"));
    const content = String(
      ((request.body ?? {}) as Record<string, unknown>).content ?? ((request.body ?? {}) as Record<string, unknown>).text ?? "",
    ).trim();
    if (!content) return reply.code(400).send(fail("VALIDATION_ERROR", "评论内容不能为空"));
    const stamp = now();
    const comment = {
      id: randomUUID(),
      taskId: task.id,
    ...
    notifyParticipants(db, task, user.id, "task_commented", "任务有新评论", `${user.name} 评论了任务“${task.title}”`);
  ...
  app.get("/api/tasks/:id/activity", async (request, reply) => {
    ...
    return reply.send(ok([...logs, ...comments, ...events].toSorted((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)))));
  });
  ...
  app.get("/api/notifications", async (request, reply) => {
    ...
    if (!user) return;
    notifyOverdueTasks(db);
    const unread = (request.query as Record<string, unknown> | undefined)?.unread === "1";
    return reply.send(
      ok(
        rows(
          db,
  ...
  app.post("/api/notifications/read", async (request, reply) => {
    ...
    const b = (request.body ?? {}) as Record<string, unknown>;
    const stamp = now();
    if (b.all) run(db, "UPDATE notifications SET is_read=1,read_at=? WHERE recipient_id=?", stamp, user.id);
    else if (typeof b.taskId === "string")
      run(db, "UPDATE notifications SET is_read=1,read_at=? WHERE recipient_id=? AND task_id=?", stamp, user.id, b.taskId);
    else if (Array.isArray(b.ids))
      for (const id of b.ids)
        if (typeof id === "string")
          run(db, "UPDATE notifications SET is_read=1,read_at=? WHERE recipient_id=? AND id=?", stamp, user.id, id);
    return reply.send(ok(null));
  });
  ...
  app.post("/api/users", async (request, reply) => {
    ...
    const role = b.role === "assistant" || b.role === "viewer" ? b.role : null;
    if (!name || !role) return reply.code(400).send(fail("VALIDATION_ERROR", "请填写成员名称并选择角色"));
    const stamp = now();
    const id = randomUUID();
    ...
    return reply.code(201).send(ok({ user: { id, name, role, isActive: true, createdAt: stamp, updatedAt: stamp }, token }));
  });
  app.patch("/api/users/:id", async (request, reply) => {
    const user = requireOwner(request, reply, db, cookie);
    if (!user) return;
    const id = idOf(request);
    if (id === "owner") return reply.code(400).send(fail("VALIDATION_ERROR", "不能停用主人账户"));
    const body = (request.body ?? {}) as Record<string, unknown>;
    if (typeof body.isActive !== "boolean") return reply.code(400).send(fail("VALIDATION_ERROR", "isActive 必须是布尔值"));
    const active = body.isActive;
    const result = run(db, "UPDATE users SET is_active=?,updated_at=? WHERE id=?", active ? 1 : 0, now(), id);
    if (!result.changes) return reply.code(404).send(fail("NOT_FOUND", "成员不存在"));
    if (!active) {
  ...
  app.post("/api/users/:id/token", async (request, reply) => {
    const user = requireOwner(request, reply, db, cookie);
    if (!user) return;
    const id = idOf(request);
    if (!one(db, "SELECT id FROM users WHERE id=? AND id<>?", id, "owner")) return reply.code(404).send(fail("NOT_FOUND", "成员不存在"));
    const token = newToken();
    const stamp = now();
  ...
  app.get("/api/todos", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    return reply.send(
      ok(
        rows(
          db,
  ...
  app.post("/api/todos", async (request, reply) => {
    ...
    if (!content) return reply.code(400).send(fail("VALIDATION_ERROR", "待办内容不能为空"));
    const stamp = now();
    const id = randomUUID();
    run(
      db,
    ...
    return reply.code(201).send(ok(findTodo(db, id)));
  });
  app.patch("/api/todos/:id", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    const id = idOf(request);
    const c = findTodo(db, id);
    if (!c) return reply.code(404).send(fail("NOT_FOUND", "待办不存在"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    const done = b.isCompleted === undefined ? Number(c.isCompleted) : b.isCompleted ? 1 : 0;
    const stamp = now();
    run(
      db,
      "UPDATE todos SET content=?,todo_date=?,is_completed=?,completed_at=?,updated_at=? WHERE id=?",
      String(b.content ?? c.content),
      b.todoDate === undefined ? c.todoDate : date(b.todoDate),
      done,
      done ? (c.completedAt ?? stamp) : null,
      stamp,
      id,
    );
    return reply.send(ok(findTodo(db, id)));
  });
  app.delete("/api/todos/:id", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    run(db, "DELETE FROM todos WHERE id=?", idOf(request));
    return reply.send(ok(null));
  });
  ...
  app.post("/api/notes", async (request, reply) => {
    ...
    if (!content) return reply.code(400).send(fail("VALIDATION_ERROR", "笔记内容不能为空"));
    const stamp = now();
    const id = randomUUID();
    run(db, "INSERT INTO notes(id,content,is_pinned,created_at,updated_at) VALUES(?,?,?,?,?)", id, content, 0, stamp, stamp);
    return reply.code(201).send(ok(findNote(db, id)));
  });
  app.patch("/api/notes/:id", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    const id = idOf(request);
    const c = findNote(db, id);
    if (!c) return reply.code(404).send(fail("NOT_FOUND", "笔记不存在"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    run(
      db,
      "UPDATE notes SET content=?,is_pinned=?,updated_at=? WHERE id=?",
      String(b.content ?? c.content),
      b.isPinned === undefined ? Number(c.isPinned) : b.isPinned ? 1 : 0,
      now(),
      id,
    );
    return reply.send(ok(findNote(db, id)));
  });
  app.delete("/api/notes/:id", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    run(db, "DELETE FROM notes WHERE id=?", idOf(request));
    return reply.send(ok(null));
  });
  app.get("/api/files", async (request, reply) => {
    ...
    const params: string[] = [];
    if (search) {
      cond.push("(name LIKE ? OR file_path LIKE ?)");
    ...
    return reply.send(
      ok(
        rows(
          db,
          `SELECT id,name,file_path AS filePath,category,last_used_at AS lastUsedAt,created_at AS createdAt,updated_at AS updatedAt FROM important_files ${cond.length ? `WHERE ${cond.join(" AND ")}` : ""} ORDER BY COALESCE(last_used_at,updated_at) DESC`,
          ...params,
        ),
      ),
    );
  ...
  app.post("/api/files", async (request, reply) => {
    ...
    const b = (request.body ?? {}) as Record<string, unknown>;
    const name = String(b.name ?? "").trim();
    const filePath = String(b.filePath ?? "").trim();
    if (!name || !filePath) return reply.code(400).send(fail("VALIDATION_ERROR", "文件名称和路径不能为空"));
    const stamp = now();
    const id = randomUUID();
    run(
      db,
    ...
    return reply.code(201).send(ok(findFile(db, id)));
  });
  app.post("/api/files/:id/use", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    const id = idOf(request);
    if (!findFile(db, id)) return reply.code(404).send(fail("NOT_FOUND", "文件不存在"));
    run(db, "UPDATE important_files SET last_used_at=?,updated_at=? WHERE id=?", now(), now(), id);
    return reply.send(ok(findFile(db, id)));
  });
  app.delete("/api/files/:id", async (request, reply) => {
    if (!requireOwner(request, reply, db, cookie)) return;
    run(db, "DELETE FROM important_files WHERE id=?", idOf(request));
    return reply.send(ok(null));
  });
  app.get("/api/review", async (request, reply) => {
    ...
    if (typeof q.ownerId === "string") {
      if (q.ownerId === "unassigned") cond.push("t.owner_id IS NULL");
      else {
        cond.push("t.owner_id=?");
        params.push(q.ownerId);
      }
    }
    if (typeof q.status === "string" && isStatus(q.status)) {
      cond.push("t.status=?");
  ...
  app.post("/api/inbox/preview", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    if (user.role === "viewer") return reply.code(403).send(fail("FORBIDDEN", "查看者不能导入任务"));
    const drafts = Array.isArray(((request.body ?? {}) as Record<string, unknown>).drafts)
      ? (request.body as { drafts: Record<string, unknown>[] }).drafts
      : [];
    return reply.send(
      ok(
  ...
  app.post("/api/inbox/import", async (request, reply) => {
    const user = require(request, reply, db, cookie);
    if (!user) return;
    if (user.role === "viewer") return reply.code(403).send(fail("FORBIDDEN", "查看者不能导入任务"));
    const b = (request.body ?? {}) as Record<string, unknown>;
    ...
    const stamp = now();
    for (const item of drafts.slice(0, 100) as Record<string, unknown>[]) {
      const title = String(item.title ?? "").trim();
      if (!title) continue;
      const fp = inboxFingerprint(item, title);
      if (one(db, "SELECT id FROM tasks WHERE wecom_fingerprint=?", fp) && !b.allowDuplicates) {
      ...
      if (t) created.push(t);
      if (owner.user?.role === "assistant" && owner.user.id !== user.id)
        notify(db, [owner.user.id], user.id, id, "task_assigned", "收到企微任务", `你收到企微任务“${title}”`);
    }
    return reply.code(201).send(ok({ created, skipped }));
  });
}
...
function requireOwner(request: FastifyRequest, reply: FastifyReply, db: Db, cookie: string): User | null {
  ...
  if (!user) return null;
  if (user.role !== "owner") {
    void reply.code(403).send(fail("FORBIDDEN", "只有主人可以访问此功能"));
    return null;
  }
  return user;
}
function userByToken(db: Db, token: string): User | null {
  const row = one(
    db,
...
function session(db: Db, userId: string): string {
  const raw = newToken();
  run(
    db,
...
function validateOwner(db: Db, id: string | null): { valid: true; user?: User } | { valid: false; message: string } {
...
function notify(db: Db, recipients: string[], actor: string, taskId: string | null, type: string, title: string, message: string): void {
...
function notifyParticipants(db: Db, t: TaskView | null, actor: string, type: string, title: string, message: string): void {
  if (!t) return;
  notify(
    db,
...
function log(db: Db, taskId: string, user: User, content: string, progress: number | null): void {
  run(
    db,
...
function notifyOverdueTasks(db: Db): void {
  ...
  for (const item of overdue) {
    ...
    run(db, "UPDATE tasks SET overdue_notified_at=? WHERE id=?", now(), item.id);
  }
}
function findTodo(db: Db, id: string): any {
  return one(
    db,
...
function findNote(db: Db, id: string): any {
...
function findFile(db: Db, id: string): any {
  return one(
    db,
...
function ownerIdOf(value: unknown, fallback: string | null): string | null {
...
function inboxFingerprint(item: Record<string, unknown>, title: string): string {
...
function rows(db: Db, sql: string, ...params: unknown[]): any[] {
